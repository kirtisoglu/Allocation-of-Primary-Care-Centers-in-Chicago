...
def test_spanningtree(spanningtree_with_forced_root):
    ...
    accumulated_candidates = {node for node in tree.graph.nodes if tree.graph.nodes[node]['candidate']>0}
    ...
    def test_find_edge_cuts(spanning_tree_with_forced_true):
        subset assigned_teams pop
        {8, 9, 10, 14}, 
        {18,19,20}
