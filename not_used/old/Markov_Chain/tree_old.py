...
# Main functions we call in recomb
...
""" MAIN FUNCTION: Bipartition of Tree
    ...
    are tried until node_repeats in which case a new tree is drawn. (if a root fails, we should generate a new set of open facilities.)
...
def random_spanning_tree(
    ...
    spanning_tree = tree.maximum_spanning_tree(graph, algorithm="kruskal", weight="random_weight")
...
class PopulatedGraph:
    ...
    def __init__(
        ...
        self.population = {node: graph.nodes[node][pop_attr] for node in graph.nodes}
    ...
    def contract_node(self, node, parent) -> None:
        ...
        #nx.contracted_nodes(self.graph, parent, node, self_loops=False)
...
def find_balanced_edge_cuts_memoization(
    ...
    while stack:
        ...
        if next_node not in subtree_pops:
            if next_node in succ: # if it is not a leaf
...
def recursive_tree_part(
    ...
    """
    Uses :func:`bipartition_tree` recursively to partition a tree into
    ``len(centers)`` parts of population ``pop_target`` (within ``epsilon``). Used to
    generate initial solution, i.e, a balanced partition.
    
    :param graph: The graph to partition into ``len(centers)`` parts.
    :type graph: nx.Graph
    :param epsilon: How far (as a percentage of ``pop_target``) from ``pop_target`` the parts
        of the partition can be.
    :type epsilon: float
